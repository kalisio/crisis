...
program
    ...
    .option('-o, --output-databases [file]', 'Where the list of output organisation databases to be deleted should be generated', 'database.json')
...
async function readDatabases () {
	if (program.inputDatabases) {
  	...
  	if (!path.isAbsolute(databasesFile)) databasesFile = path.join(process.cwd(), databasesFile)
		...
		console.log(`Found ${databases.length} organisation databases in application database`)
	}
}
...
async function filterDatabases() {
	...
	nbNoOrgDatabase += _.remove(databases, (database) => {
		try {
			const id = new mongodb.ObjectId(database.name)
			return !id.isValid() // If we can create a valid ObjectID it is in the right format
		} catch (error) {
...
// Export databases not found in app DB to file then remove it from app DB as well if it is required
