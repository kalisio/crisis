...
then
  ...
  # Retrieve the built Web app
	#
	travis_fold start "download-app-artifact"
	aws s3 sync s3://$APP-builds/$TRAVIS_BUILD_NUMBER/dist cordova/www > /dev/null
  travis_fold end "download-app-artifact"
  ...
  #
	...
	#
  travis_fold start "provision-certificates"
  ...
  travis_fold end "provision-certificates"
  
	#
	# Build the app
	#
	travis_fold start "build-app"
  # Increment the build number
	...
	travis_fold end "build-app"
  
	#
  # Deploy the IPA to the AppleStore
	#
	travis_fold start "deploy-ipa"
