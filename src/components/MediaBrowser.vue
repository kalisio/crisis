  ...
  >
    ...
    <template v-for="(media) in medias" :key="media.key">
      <q-carousel-slide :name="media.name" class="row justify-center items-center">
        ...
        <ImageViewer :ref="media.uri" class="fit k-media-browser-slide" :source="media.uri" :interactive="media.isImage" @image-transformed="onImageTransformed" />
      </q-carousel-slide>
    </template>
    <!--
      Empty slide
...
import { utils as kCoreUtils, Storage, Events } from '@kalisio/kdk/core.client'
...
export default {
  ...
  methods: {
    ...
    async onDownloadMedia () {
      ...
      kCoreUtils.downloadAsBlob(data.buffer, this.currentMedia.name, data.type)
    },
    async onUploadMedia () {
